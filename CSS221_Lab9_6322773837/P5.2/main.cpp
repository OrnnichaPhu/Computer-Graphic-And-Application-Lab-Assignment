...
float t=0, dt=0.009;
float t1=0, dt1=0.009;

double shiftx,shifty,shiftz;//position of the triangle
double shiftx2,shifty2,shiftz2;

double xp[]={-2,-1,0,1,2,2,2};
double yp[]={1,1,1,1,1,2,3};
double zp[]={-2,-2,-2,-2,-2,-2,-2};
...
{return f(zp,t);} //- z interpolation
//----------------------------------------------
void Trajectory1()//changes global variables shiftx, shifty, shiftz to move the triangle
{
    shiftx=x1(t);
    shifty=y1(t);
    shiftz=z1(t);
...
// t1=t1+dt1;
         //if ((fabs(t1>=n-1)) || (t1<=0)) {glTranslatef(2,2,-2); glRotated(ang,0,1,0);}
     glRotated(sang,0,1,0);
 glColor3d(1,1,0);
 glutWireSphere(0.5,40,40);
 glColor3d(0,0,1);
